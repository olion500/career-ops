...
- `/Users/2302-pc04/space/docs/02_Areas/기업/` — 33 researched company `.md` files (plus `회사.loom` and `00_apply_2025.base` which are excluded) with structured frontmatter (company, field, location, remote, stack, job, url, rating, status, open) plus sections 회사/제품/평판/직무/평가
...
├── portals.yml                           ← 33 companies + 3 boards
├── data/
...
- `/Users/2302-pc04/space/docs/02_Areas/기업/*.md` (33 files; exclude `.loom`, `.base`)
...
| `portals.yml` | `기업/*.md` frontmatter | 33 companies + 원티드/점핏/지킹 |
...
5. Claude: validates outputs (cv.md present, profile.yml valid YAML,
   portals.yml has 33 companies, no slug collisions, tracker has 4 entries)
...
- [ ] `portals.yml` has 33 companies + 3 boards, all with valid URLs
- [ ] `data/applications.md` has 4 historic entries with Korean statuses
- [ ] `data/companies/` has 33 .md files matching slugs
- [ ] `data/companies/_slugs.yml` covers all 33 companies
